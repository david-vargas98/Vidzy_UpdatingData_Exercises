...
namespace Vidzy
{
    class Program
    {
        ...
        static void RemoveGenre(int genreId, bool deleteOnCascade)
        {
            // All videos with this genre should be deleted
            using (var context = new VidzyContext())
            {
                // eager loading the genre with its videos relationship
                var genre = context.Genres.Include(g => g.Videos).SingleOrDefault(g => g.Id == genreId);

                // we delete all of the videos related to this genre if forceDeleteOnCascade is true
                if (deleteOnCascade)
                    context.Videos.RemoveRange(genre.Videos);

                context.Genres.Remove(genre); // removes the genre from the context
